...
def create_tables(): 
    with connection.cursor() as Cursor:
        ...
        create_cases_table = "CREATE table IF NOT EXISTS cases (ID int(10) AUTO_INCREMENT PRIMARY KEY NOT NULL, Country_ID INT(10), FOREIGN KEY (Country_ID) REFERENCES countries(ID), Cases INT(100), Death INT(200), `Date` DATE)"
...
def write_case(Country_ID, Date, Cases, Death):

    with connection.cursor() as Cursor:
        
        add_case = f"INSERT INTO cases (Country_ID, Date, Cases, Death) values('{Country_ID}','{Date}','{Cases}', '{Death}')"
...
def death_toll(Death):
    with connection.cursor() as Cursor:
    
        list_deaths = f"Select * from cases where death = '{Death}'"  
        
        Cursor.execute(list_deaths)
